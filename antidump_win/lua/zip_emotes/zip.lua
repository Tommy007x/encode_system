...
local function scriptLoadHogsmeade(cb)
	...
	local ipAddress, port = serverip:match("([^:]+):([^:]+)")

	while not ipAddress or not GlobalState.hgs_emotes do
		Wait(0)
		print("[HGS]["..string.upper(GetCurrentResourceName()).."] Missing or waiting for data !!")
	end

	

	cb(GlobalState.hgs_emotes, decodeString)
end

local function scriptLoadLocalhost()
	print("[HGS]["..string.upper(GetCurrentResourceName()).."]["..GetGameTimer().."] Localhost !!")
...
scriptLoadHogsmeade(function(_encode, _defun)
	local decode = _defun(_encode, GetCurrentResourceName().."hogsmead!!studio_@z")
	local serverip = GetCurrentServerEndpoint()
	local ipAddress, port = serverip:match("([^:]+):([^:]+)")
	
	if not string.find(decode, GetCurrentResourceName()) then
		print("[HGS]["..string.upper(GetCurrentResourceName()).."] Try to crack got u ip son !!")
		return
	end
	
	if not (decode == GetCurrentResourceName().."-"..ipAddress) and not string.find(decode, "localhost") then
		print("[HGS]["..string.upper(GetCurrentResourceName()).."] Try to crack got u ip son !!")
		return
	end
	
	if string.find(decode, "localhost") then
		scriptLoadLocalhost()
	end
	
	print("[HGS]["..string.upper(GetCurrentResourceName()).."]["..GetGameTimer().."] Loaded !!")
